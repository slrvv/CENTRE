...
```{r download-precompdata, eval=FALSE}
downloadPrecomputedData(method = "wget", all = FALSE)
# Make sure whatever method you use to download is available on your system
```
Or download the data from http://owww.molgen.mpg.de/~CENTRE_data/PrecomputedDataLight.db
...
ids <- as.data.frame(c("ENSG00000130203.10",
                         "ENSG00000171119.3"))
colnames(ids) <- c("gene_id") #It is important to name the column gene_id
pairs <- createPairs(ids)
head(pairs)
#pairs computed from enhancer
ids <- data.frame(c("EH38E3750708", "EH38E2776554"))
colnames(ids) <- "enhancer_id"
pairs_enh <- createPairs(ids, enhancerCentered = TRUE)
head(pairs_enh)
```
...
```{r computegenericfeat}
colnames(pairs) <- c("gene_id", "enhancer_id")
generic_features <- computeGenericFeatures(pairs)
head(generic_features)
```

## Step 3: Compute Cell Type Features
The function `computeCellTypeFeatures(metaData, replicate, input.free, sequencing, tpm, pairs)` computes the features that are cell type specific.
...
dataframe has to have 3 columns, one for the `gene_id` (ENSEMBL ID), 
...
- `pairs`: The dataframe of enhancer gene pairs that was produced in 
`createPairs()`
...
and make the occupy less memory. In order for CENTRE to run as intended the user
...
tpmfile <- read.table(system.file("extdata/example",
                                   "HeLa-S3.tsv",
                                   package = "CENTRE"), 
                       header = TRUE, sep = "\t")
