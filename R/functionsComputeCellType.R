...
createRegionsDf <- function(listProm, listEnh, pairs) {
    ...
    # add distance value to sort start and end for regulatory distance
    # calculations.
    regions$distance <- regions$middle_point - regions$transcription_start
    return(regions)
}

#' @description Get the CRUP scores (Enhancer Probability or Promoter
#' probability)
#' for each enhancer.
#'
...
computeCrupEnhancer <- function(regionsEnhancer,
    ...
    enhancerMetadata <- GenomicRanges::elementMetadata(enhancerRanges)
    cresEP$enhancer_id <- enhancerMetadata$enhancer_id[cresEP$cres]

    if (promprob == TRUE) {
        # get the PP crup scores
        crupMetadata <- GenomicRanges::elementMetadata(crupScores)
        cresEP$PP_prob_enh <- crupMetadata$probP[cresEP$EP]
        ...
        crupMetadata <- GenomicRanges::elementMetadata(crupScores)
        cresEP$EP_prob_enh <- crupMetadata$prob[cresEP$EP]
        cresEP$enhancer_id <- factor(cresEP$enhancer_id)
        cresEP$bin <- rep(1:5, times = length(enhancerRanges))
...
computeCrupPromoter <- function(regionsProm,
    ...
    genesMetadata <- GenomicRanges::elementMetadata(geneRanges)
    cresEP$gene_id1 <- genesMetadata$gene_id1[cresEP$promoter]

    if (promprob == TRUE) {
        crupMetadata <- GenomicRanges::elementMetadata(crupScores)
        cresEP$PP_prob_gene <- crupMetadata$probP[cresEP$EP]
        ...
        crupMetadata <- GenomicRanges::elementMetadata(crupScores)
        cresEP$EP_prob_gene <- crupMetadata$prob[cresEP$EP]
        cresEP$gene_id1 <- factor(cresEP$gene_id1)
        cresEP$bin <- rep(1:5, length(geneRanges))
...
createBetweenRanges <- function(regions) {
    ...
    ## tss > middle point.
    ## in the positive case bstart should be the tts and in the negative it should
    ## be  the middle point
    regions$bstart <- regions$middle_point ## assign every value to the middle
    # point
...
computeCrupRegDistanceEnh <- function(input, prediction, betweenRanges) {
    ...
    cresEP <- data.frame(
        between = betweenMetadata$pair[hitsEnh@from],
        EP_reg_distance = predMetadata$prob[hitsEnh@to]
    )
...
computeCrupRegDistanceProm <- function(input, prediction, betweenRanges) {
    ...
    predMetadata <- GenomicRanges::elementMetadata(prediction)
    cresPP <- data.frame(
        between = betweenMetadata$pair[hitsProm@from],
        PP_reg_distance = predMetadata$probP[hitsProm@to]
    )
    ...
    if (nrow(cresPP1) != 0) {
        ...
        # avoid uncommon edge case in which all ET pairs have EP_reg_distance above
        # 0.5 is 0
...
getEPFeatures <- function(regions, crupScores, pairs) {
    ...
    crupEPprom <- computeCrupPromoter(
        regions,
        crupScores
    )
    ...
    crupEPFeatures <- computeCrupRegDistanceEnh(
        crupEPFeatures,
        crupScores,
        betweenRanges
    )
...
getPPFeatures <- function(regions, crupScores, crupEPFeatures) {
    ...
    # create the betweenRanges objects that is used for the distance calculations
    betweenRanges <- createBetweenRanges(regions)
    crupFeatures <- computeCrupRegDistanceProm(
        crupFeatures,
        crupScores,
        betweenRanges
    )
